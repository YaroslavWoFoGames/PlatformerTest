...
namespace Platformer.Core
{
	public class GameStateMachine : ITickable, IDisposable
	{
		...
		public void SwitchState<T>(bool shouldRemoveFromCache) where T : IGameState, new()
		{
		...
		private void ChangeState(IGameState? gameState)
		{
			...
			if (_currentState != null)
			{
				_currentState.SetStateMachine(this, _container);
				_currentState.Enter();
			}
		}

		private T CreateState<T>(bool shouldRemoveFromCache) where T : IGameState, new()
		{
			var key = typeof(T);
